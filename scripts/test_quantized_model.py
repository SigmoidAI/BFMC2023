...
DET_MODEL_NAME = "best_based_on_nano"
det_model = YOLO(f'./models/{DET_MODEL_NAME}.pt')
...
def run_object_detection(source=0, flip=False, use_popup=True, skip_first_frames=0, model=det_model, device=device):
    player = None
    if device != "CPU":
...
det_model_path = Path(f"./models/{DET_MODEL_NAME}_openvino_model/{DET_MODEL_NAME}.xml")
...
run_object_detection(source='./files/qualification_video.mp4', flip=False, use_popup=True, model=det_ov_model, device="AUTO")